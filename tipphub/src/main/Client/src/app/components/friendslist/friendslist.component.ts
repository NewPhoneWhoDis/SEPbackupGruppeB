...
export class FriendslistComponent implements OnInit {
      ...
      .subscribe((data) => {
        ...
        this.age = Math.abs(
          new Date(Date.now()).getUTCFullYear() -
            //@ts-ignore
            new Date(data.dateOfBirth).getUTCFullYear()
        );
      });
  }
  ...
  addFriend(email: String): void {
    ...
    this.friendslistService.sendFriendRequest(friendRequest, email).subscribe({
      ...
      error: () => {
        window.alert(
          "Du hast schon eine Freundschaftsanfrage diesem Nutzer geschickt!"
        );
        window.location.reload();
      },
    });
  ...
  deleteFriend(friendId: number | undefined): void {
    this.friendslistService
      .deleteFriend(this.currentUser?.id, friendId)
      .subscribe((data) => {
        window.location.reload();
      });
  }
  ...
  requestBetPermission() {
    this.notificationService
      .requestBetPermission(this.storageService.getLoggedUser())
      .subscribe({
        next: () => {
